...
#===  FUNCTION  ================================================================
#         NAME:  run_mail_test
#      PURPOSE:  just sends a little test mail to $addr
#  DESCRIPTION:  
...
sub run_mail_test
{
   ...
   #my $mail = new();
   $mail ->add_recipient($addr);
   #$mail ->set_from("STB health data aggregator <cron@".`hostname`.">");
   my $hostname = `hostname`;
   chomp $hostname;
   $mail ->set_from("STB Health data aggregator <cron\@$hostname>");
   $mail ->set_subject("Testing mail capabilities");
   $mail ->msg_append("This is the test message\n Enjoy.");
...
#===  FUNCTION  ================================================================
#         NAME:  run_attachment_test
#      PURPOSE:  tests attaching a file to the email
#  DESCRIPTION:  
#   PARAMETERS:  addr - string
#      RETURNS:  n/a
#===============================================================================
sub run_attachment_test
{
   my $mail = shift;
   my $addr = shift;

   ### Adjust sender, recipient and your SMTP mailhost
   my $from_address = 'cron@linux-pc-192';
   my $to_address = $addr;
   my $mail_host = "localhost";

   ### Adjust subject and body message
   my $subject = "Monthly IPTV Downlowds. Models Report.";
   ...
   $msg->attach (
     Type => 'TEXT',
     Data => "This is the monthly models report.\nenjoy.\n",
   ) or die "Error adding the text message part: $!\n";

   $msg->attach (
      Path => "./MontlyModels.csv",
      Filename => "MontlyModels.csv",
      Disposition => 'attachment'
   ) or die "Error adding MontlyModels.csv: $!\n";
